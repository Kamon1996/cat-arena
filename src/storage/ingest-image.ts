 ...
 * prefetched original when they already downloaded it for the dedupe check.
 */
export async function ingestImage(imageId: string, prefetched?: Buffer): Promise<ScreenedImage> {
  const { width, height, screenBuffer, sha256 } = await processImage(imageId, prefetched);
